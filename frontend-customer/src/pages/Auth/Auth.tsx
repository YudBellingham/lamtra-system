...
const Auth = () => {
      ...
      if (session?.user) {
        try {
          ...
          if (!fetchError && !existing) {
            const { data: newCust, error: oError } = await supabase
              .from("customers")
              .insert({
            ...
            if (!oError && newCust) {
              const { data: welcomeVouchers } = await supabase
                .from("vouchers")
                .select("*")
                .eq("iswelcome", true);
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!emailRegex.test(regEmail)) {
      setErrorMsg(
        "Email không hợp lệ. Vui lòng kiểm tra định dạng và chính tả (vd: @gmail.com).",
      );
      setLoading(false);
      return;
    }
    ...
    try {
      const checkRes = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/auth/check-existence`,
        {
          email: regEmail,
          phone: phone,
        },
      ...
      <div className="auth-page">
        ...
        >
          <form
            ...
            style={{ width: "100%", padding: "0 40px", maxWidth: "450px" }}
          >
