...
import { FiUser, FiGift, FiShoppingBag, FiLock, FiLogOut, FiClock, FiHeart } from 'react-icons/fi';
...
const Profile = () => {
  ...
  const [editForm, setEditForm] = useState({ fullname: '', email: '', phone: '', birthday: '' });
  ...
  const [favorites, setFavorites] = useState<{products: any[], templates: any[]}>({ products: [], templates: [] });
  ...
  const fetchLoyaltyData = async (customerId: number) => {
    ...
    try {
      ...
      const [vouchersRes, historyRes, myVouchersRes] = await Promise.all([
        ...
        axios.get('http://localhost:8000/api/customers/point-history', { headers }),
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:8000/api/customers/update-profile', {
        customerId: customer.customerid,
  ...
  const handleReorder = async (e: React.MouseEvent, orderId: string) => {
    ...
    try {
        const res = await axios.post('http://localhost:8000/api/orders/reorder', { orderid: orderId });
        if (res.data.success) {
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const res = await axios.post('http://localhost:8000/api/orders/template/save', 
      ...
      toast.error('Lỗi khi lưu đơn mẫu: ' + (err.response?.data?.error || err.message));
    } finally {
        ...
        {!isEditingInfo ? (
          ...
          <div>
            <button 
              ...
              onClick={() => {
                setIsEditingInfo(false);
                setEditForm({ fullname: customer?.fullname, email: customer?.email, phone: customer?.phone, birthday: customer?.birthday });
              }}
            ...
            <button 
              ...
              disabled={isSavingInfo || !editForm.fullname || !editForm.phone || !/^0\d{9}$/.test(editForm.phone)}
              style={{ opacity: (isSavingInfo || !editForm.fullname || !editForm.phone || !/^0\d{9}$/.test(editForm.phone)) ? 0.6 : 1 }}
            >
        ...
        <div className="info-card">
          ...
          {!isEditingInfo ? (
            <div className="value">
                ...
                {!customer?.birthday && (
                  <div style={{ fontSize: '12px', color: '#8d6e63', marginTop: '4px', fontWeight: 'normal', fontStyle: 'italic' }}>
                      🎂 Cập nhật ngày sinh để nhận quà bất ngờ vào tháng sinh nhật của bạn!
                  </div>
                )}
            </div>
          ) : (
            <input type="date" className="profile-edit-input" value={editForm.birthday} onChange={e => setEditForm({...editForm, birthday: e.target.value})} />
          )}
        </div>
      </div>

      <h2 className="tab-head mt-4">Hạng & Tích điểm</h2>
      <div className="rank-cards-wrapper" style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
        <div className="rank-card points-card" style={{ flex: 1, minWidth: '200px', background: '#fff9fa', border: '1px solid #ffccd5', borderRadius: '12px', padding: '20px', display: 'flex', alignItems: 'center', gap: '15px' }}>
          <div className="rank-icon"><Coins size={36} color="#d81b60" /></div>
          <div className="rank-details">
            <label style={{ display: 'block', fontSize: '13px', color: '#666', marginBottom: '5px' }}>Ví điểm khả dụng</label>
            <div className="rank-val" style={{ color: '#d81b60', fontSize: '24px', fontWeight: 'bold' }}>{customer?.totalpoints || 0} điểm</div>
          </div>
        </div>
        <div className={`rank-card tier-card tier-${(customer?.membership || 'Đồng').toLowerCase()}`} style={{ flex: 1, minWidth: '200px', background: '#f5f7ff', border: '1px solid #cce0ff', borderRadius: '12px', padding: '20px', display: 'flex', alignItems: 'center', gap: '15px' }}>
          <div className="rank-icon"><Crown size={36} color="#3366cc" /></div>
          <div className="rank-details">
            <label style={{ display: 'block', fontSize: '13px', color: '#666', marginBottom: '5px' }}>Hạng thành viên</label>
            <div className="rank-val" style={{ fontSize: '24px', fontWeight: 'bold', color: '#3366cc' }}>{customer?.membership || 'Đồng'}</div>
          </div>
        </div>
      </div>

      {customer && (
        <div className="progress-section" style={{ marginTop: '25px', padding: '20px', background: '#fcfcfc', borderRadius: '12px', border: '1px solid #eee' }}>
           <h3 style={{ margin: '0 0 15px 0', fontSize: '16px', color: '#333' }}>Tiến trình thăng hạng</h3>
           {(() => {
              const info = (() => {
                 ...
                 if (pts < 100) {
                    ...
                    return { isMax: true, percentage: 100, text: 'Bạn đang ở hạng cao nhất' };
                 }
              ...
              return (
                 <div>
                     <p style={{ margin: '0 0 10px 0', fontSize: '14px', color: '#555', fontWeight: '500' }}>
                        {info.text}
                     </p>
                     <div style={{ width: '100%', height: '10px', background: '#e0e0e0', borderRadius: '5px', overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${info.percentage}%`, background: 'linear-gradient(90deg, #ff8a00, #e52e71)', transition: 'width 0.5s ease-in-out' }}></div>
                     </div>
                     <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px', fontSize: '12px', color: '#888', fontWeight: 'bold' }}>
                        <span>Đồng (0)</span>
                        <span>Bạc (100)</span>
                        <span>Vàng (500)</span>
                     </div>
    ...
    <div className="tab-pane fade-in">
      ...
      <div className="voucher-grid" style={{ display: 'flex', overflowX: 'auto', scrollSnapType: 'x mandatory', gap: '15px', paddingBottom: '10px' }}>
        {vouchers.map(v => {
          ...
          <div className={`voucher-item`} style={{ flex: '0 0 calc(50% - 10px)', scrollSnapAlign: 'start', filter: isNotEnough ? 'grayscale(100%) opacity(0.6)' : 'none' }} key={v.voucherid}>
            ...
            >
              {isNotEnough ? `Cần thêm ${v.pointsrequired - customer?.totalpoints} điểm` : (redeemLoading ? 'Đang đổi...' : 'Đổi quà')}
            </button>
          </div>
        )})}
      ...
      <div className="voucher-grid" style={{ marginBottom: '30px', display: 'flex', overflowX: 'auto', scrollSnapType: 'x mandatory', gap: '15px', paddingBottom: '10px' }}>
         {myVouchers.map(mv => {
            ...
            if (!v) return null;
            return (
               <div key={mv.custvoucherid} className="voucher-item" style={{ flex: '0 0 calc(50% - 10px)', scrollSnapAlign: 'start', border: '1px solid #4caf50', background: '#f1f8e9', position: 'relative' }}>
                 {mv.count > 1 && (
                 ...
                 <div className="v-value" style={{ color: '#2e7d32', paddingRight: mv.count > 1 ? '30px' : '0', fontSize: '15px', fontWeight: 'bold' }}>
                    Giảm {v.discounttype === '%' ? `${v.discountvalue}%` : formatPrice(v.discountvalue)}
                 </div>
                 <div className="v-title" style={{ color: '#1b5e20', fontSize: '13px', margin: '4px 0', fontWeight: '500' }}>{v.title}</div>
                 <div style={{ fontSize: '11px', color: '#666', marginBottom: '8px' }}>
                    Đơn tối thiểu {formatPrice(v.minordervalue || 0)} 
                    {v.discounttype === '%' && v.maxdiscount && ` - Giảm tối đa ${formatPrice(v.maxdiscount)}`}
                 </div>
                 <div className="v-points" style={{ color: '#388e3c', fontSize: '11px', borderTop: '1px dashed #c8e6c9', paddingTop: '5px' }}>
                    HSD: {v.expirydate ? new Date(v.expirydate).toLocaleDateString('vi-VN') : 'Không giới hạn'}
                 </div>
                 <button className="btn-redeem" style={{ background: '#4caf50' }} onClick={() => navigate('/san-pham')}>Dùng ngay</button>
               </div>
            );
         })}
         {myVouchers.length === 0 && <p className="empty-text">Bạn chưa sở hữu voucher nào.</p>}
      </div>

      <h2 className="tab-head mt-4">Lịch sử điểm</h2>
      <div style={{ display: 'flex', gap: '10px', marginBottom: '15px', flexWrap: 'wrap', alignItems: 'center' }}>
         <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={() => setHistoryFilter('all')} style={{ padding: '6px 15px', borderRadius: '20px', border: '1px solid #d81b60', background: historyFilter === 'all' ? '#d81b60' : 'transparent', color: historyFilter === 'all' ? '#fff' : '#d81b60', cursor: 'pointer', fontWeight: 'bold', fontSize: '13px' }}>Tất cả</button>
            <button onClick={() => setHistoryFilter('in')} style={{ padding: '6px 15px', borderRadius: '20px', border: '1px solid #4caf50', background: historyFilter === 'in' ? '#4caf50' : 'transparent', color: historyFilter === 'in' ? '#fff' : '#4caf50', cursor: 'pointer', fontWeight: 'bold', fontSize: '13px' }}>Nhận điểm</button>
            <button onClick={() => setHistoryFilter('out')} style={{ padding: '6px 15px', borderRadius: '20px', border: '1px solid #f44336', background: historyFilter === 'out' ? '#f44336' : 'transparent', color: historyFilter === 'out' ? '#fff' : '#f44336', cursor: 'pointer', fontWeight: 'bold', fontSize: '13px' }}>Dùng điểm</button>
         </div>
         <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '13px', color: '#666' }}>Lọc theo ngày:</span>
            <input type="date" value={filterDate} onChange={e => setFilterDate(e.target.value)} style={{ padding: '5px 10px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '13px' }} />
            {filterDate && <button onClick={() => setFilterDate('')} style={{ background: 'none', border: 'none', color: '#d81b60', cursor: 'pointer', fontSize: '13px', textDecoration: 'underline' }}>Xóa</button>}
         </div>
      </div>
      <div className="history-list" style={{ maxHeight: '400px', overflowY: 'auto', paddingRight: '5px', border: '1px solid #f9f9f9', borderRadius: '10px' }}>
        {pointHistory.filter(h => {
             ...
             let matchDate = true;
             if (filterDate) {
                matchDate = new Date(h.createddate).toISOString().split('T')[0] === filterDate;
             }
          ...
          <div className="history-item" key={h.pointhistoryid}>
            ...
            <div className="h-right" style={{ fontWeight: 'bold', fontSize: '16px', color: h.pointchange > 0 ? '#4caf50' : '#f44336', display: 'flex', alignItems: 'center' }}>
              {h.pointchange > 0 ? '+' : ''}{h.pointchange} điểm
            </div>
          </div>
        ))}
        ...
        {orders.map(o => {
          ...
          return (
            ...
            >
              ...
              <div className="o-footer" style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', width: '100%' }}>
                {o.status === 'Hoàn thành' ? (
                  <button 
                    className="btn-view-order" 
                    onClick={(e) => handleReorder(e, o.orderid)}
                    disabled={reorderLoading === o.orderid}
                    style={{ background: '#ffebf0', color: '#d81b60', border: '1px solid #ffccd5' }}
                  >
                  ...
                  <button 
                    ...
                    style={{ background: '#f5f5f5', color: '#aaa', border: '1px solid #ddd', cursor: 'not-allowed' }}
                  >
                    Đặt lại
                  </button>
                )}
        ...
        <div className="voucher-grid" style={{ marginBottom: '30px' }}>
            {favorites.products.map(fav => (
                <div key={fav.id} className="voucher-item" style={{ border: '1px solid #ffeff3' }}>
                    ...
                    <button className="btn-redeem" onClick={() => navigate(`/san-pham/${fav.productid}`)}>Xem món</button>
                </div>
            ))}
            {favorites.products.length === 0 && <p className="empty-text">Bạn chưa thích món nào.</p>}
        </div>
    ...
    <div className="profile-page">
      ...
      {errorMsg && (
          <div className="toast-msg toast-err">
            {errorMsg}
            <button onClick={() => setErrorMsg('')} style={{ background: 'none', border: 'none', color: 'inherit', marginLeft: '10px', cursor: 'pointer', fontWeight: 'bold' }}>✕</button>
          </div>
      )}
      {successMsg && (
          <div className="toast-msg toast-succ">
            {successMsg}
            <button onClick={() => setSuccessMsg('')} style={{ background: 'none', border: 'none', color: 'inherit', marginLeft: '10px', cursor: 'pointer', fontWeight: 'bold' }}>✕</button>
          </div>
      )}
      ...
      <div className="profile-container">
        ...
        <aside className="profile-sidebar">
          ...
          <div className="sidebar-menu">
            ...
            <button className={`sidebar-item ${activeTab === '5' ? 'active' : ''}`} onClick={() => setSearchParams({ tab: '5' })}>
      ...
      {showTemplateModal && (
        <div className="template-modal-overlay" onClick={() => setShowTemplateModal(false)}>
          <div className="template-modal" onClick={e => e.stopPropagation()}>
            ...
            <p>Đặt tên cho combo này để nhanh chóng đặt lại trong lần sau nhé!</p>
