...
const PaymentResult: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // If neither VNPay response nor COD orderId, redirect home
    if (!txnRef && !codOrderId) {
      navigate("/");
      return;
    }
    ...
    // Determine which ID to use (VNPay or COD)
    const orderIdToUse = txnRef || codOrderId;
    ...
    const updateOrder = async () => {
      try {
        ...
        // COD order - already has status "Chờ xác nhận" from checkout
        else if (codOrderId) {
          clearCart();
          setStatus("success");

          setTimeout(() => {
            navigate(`/order/${codOrderId}`);
          }, 3000);
        }
      } catch (error) {
      ...
      <div className="payment-card">
        {status === "success" ? (
          <>
            ...
            <p>
              Đơn hàng <strong>{orderId}</strong> của bạn đang được xử lý. Bạn
              sẽ được chuyển hướng sau 3 giây...
            </p>
          ...
          <>
            ...
            <p>
              Giao dịch của <strong>{orderId}</strong> đã bị hủy hoặc có lỗi xảy
              ra. Vui lòng quay lại giỏ hàng để sửa đổi hoặc thử lại phương thức
              khác.
            </p>
            <div className="result-actions">
