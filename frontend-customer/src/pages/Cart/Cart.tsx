...
const Cart: React.FC = () => {
    ...
    const fetchVouchers = async () => {
      ...
      try {
        ...
        if (customer) {
          ...
          if (cv) {
            const rawVouchers = cv
              ?.map((item) => item.vouchers)
              ?.filter((v) => v);
            const grouped: any = {};
    ...
    const checkPhone = async () => {
      ...
      if (session?.user) {
        ...
        if (data) {
          ...
          if (!formData.fullName && data.fullname)
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (addedCount === 0) {
          ...
          if (hasMissingItems) {
            toast.success(
              `Đã thêm ${addedCount} món vào giỏ. Một số món đã ngừng phục vụ.`,
              { id: toastId },
            );
          } else {
            toast.success("Đã nạp combo yêu thích vào giỏ hàng!", {
              id: toastId,
            });
          }
        }
      ...
      toast.error(
        "Lỗi khi nạp đơn mẫu: " + (err.response?.data?.error || err.message),
        { id: toastId },
      );
    } finally {
  ...
  const handleAddressInput = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await axios.get(
        `https://rsapi.goong.io/Place/AutoComplete?api_key=${apiKey}&input=${encodeURIComponent(val)}`,
      );
  ...
  const estimateShipping = async (lat?: number, lng?: number) => {
    ...
    if (orderType === "Giao hàng" && targetLat && targetLng) {
      ...
      try {
        ...
        const res = await axios.post(
          `${import.meta.env.VITE_API_URL}/api/estimate-shipping`,
          payload,
        );
        if (res.data.success) {
          const {
          ...
          if (fallbackLevel > 1) {
            setFallbackLevelWarning(
              "Đang ước tính khoảng cách dựa trên Phường/Xã do thiếu số nhà.",
            );
          }

          if (!branchesInfo || branchesInfo.length === 0) {
            setShippingError(
              `Rất tiếc, không có chi nhánh nào khả dụng trong 15km.`,
            );
          } else {
        ...
        setShippingError(
          err.response?.data?.error ||
            "Lỗi máy chủ khi tính khoảng cách giao hàng",
        );
      } finally {
  ...
  const handleBranchChange = async (
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/checkout/capability`,
        { branchId: bid, cart },
      );
      if (res.data.success) {
        ...
        if (!cap.available) {
          setBranchError(
            `Chi nhánh này hiện đang tạm hết nguyên liệu (${cap.outOfStockItems.join(", ")}). Vui lòng điều chỉnh giỏ hàng hoặc đổi chi nhánh.`,
          );
  ...
  const getOptionsSummary = (item: any) => {
    ...
    if (item.toppings && item.toppings.length > 0)
      summary += `, + ${item.toppings?.map((t: any) => t.name).join(", ")}`;
  ...
  const applyVoucher = (v: any) => {
    if (cartTotal < (v.minordervalue || 0)) {
      toast.error(
        `Đơn hàng tối thiểu ${formatPrice(v.minordervalue || 0)} để sử dụng mã này.`,
      );
      return;
    }
  ...
  const handleCheckoutSubmit = async () => {
    if (orderType === "Giao hàng") {
      if (
        ...
        toast.error(
          "Vui lòng chọn địa chỉ giao hàng hợp lệ từ gợi ý của hệ thống!",
        );
        return;
      }
      if (shippingError) {
      ...
      if (!formData.fullName || !formData.phone || !selectedBranchId) {
        toast.error("Vui lòng điền thông tin và chọn chi nhánh!");
        return;
      }
      if (branchError) {
        toast.error("Không thể đặt vì chi nhánh đang hết nguyên liệu.");
        return;
      }
      if (
        pickupCapability &&
        pickupCapability.available &&
        pickupCapability.isOverloaded &&
        !pickupOverloadConfirmed
      ) {
        toast.error("Vui lòng xác nhận đồng ý đợi lâu.");
        return;
      }
    }
    ...
    try {
      ...
      const statusToSave =
        formData.paymentMethod === "COD" ? "Chờ xác nhận" : "Chờ thanh toán";
      const session = (await supabase.auth.getSession()).data.session;
      let customerId = null;
      if (session?.user?.id) {
        const { data } = await supabase
      ...
      const payload = {
        ...
        customerInfo: {
          ...
          address_detail: formData.manual_address
            ? `${formData.manual_address} - ${formData.address_detail}`
            : formData.address_detail,
    ...
    return (
      <main className="cart-page">
        <div className="empty-cart">
          ...
          <p>
            Dường như bạn chưa chọn món nào. Hãy khám phá menu của Lam Trà nhé!
          </p>
      ...
      {customerPhone === "" && (
        <div
          style={{
            ...
            fontWeight: "500",
            fontSize: "14px",
            borderBottom: "1px solid #ffe9a6",
          }}
        >
          🎁 Lam Trà đang có chương trình tích điểm đổi quà hấp dẫn. Quý khách
          hãy{" "}
          <Link
            to="/profile?tab=1"
            style={{
          ...
          >
            cập nhật số điện thoại tại đây
          </Link>{" "}
          để không bỏ lỡ quyền lợi!
        </div>
      )}
            ...
            <div className="checkout-form-container">
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "8px",
                    background:
                      orderType === "Giao hàng" ? "#d81b60" : "transparent",
                    color: orderType === "Giao hàng" ? "#fff" : "#d81b60",
                    fontWeight: "bold",
                    cursor: "pointer",
                  }}
                >
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "8px",
                    background:
                      orderType === "Tại chỗ" ? "#d81b60" : "transparent",
                    color: orderType === "Tại chỗ" ? "#fff" : "#d81b60",
                    fontWeight: "bold",
                    cursor: "pointer",
                  }}
                >
                      ...
                      >
                        {addressSuggestions?.map((s) => (
                          <li
                            key={s.place_id}
                            onClick={() =>
                              handleSelectAddress(s.place_id, s.description)
                            }
                            style={{
                              padding: "12px 15px",
                              cursor: "pointer",
              ...
              {orderType === "Tại chỗ" && (
                <div className="branch-section">
                  ...
                  >
                    <option value="">
                      Chọn chi nhánh mà bạn muốn đến lấy *
                    </option>
                    {branches?.map((b) => (
                  ...
                  {pickupCapability &&
                    ...
                    !pickupOverloadConfirmed && (
                      ...
                      >
                        ...
                        >
                          ⚠️ Chi nhánh này đang rất đông đơn, thời gian chuẩn bị
                          dự kiến {pickupCapability.estimatedTime}. Bạn có chấp
                          nhận đợi không?
                        </p>
                        <button
                          onClick={() => setPickupOverloadConfirmed(true)}
                          style={{
                            padding: "8px 15px",
                            background: "#d81b60",
                            color: "white",
        ...
        <div className="cart-right">
          ...
          >
            ...
            {selectedVoucher ? (
              ...
              >
                <div>
                  <strong style={{ color: "#d81b60", display: "block" }}>
          ...
          <div className="summary-row total">
            <span>Tổng cộng</span>
            <span style={{ fontSize: "24px", color: "#d81b60" }}>
          ...
          {!isCheckout ? (
            ...
            <button
              ...
              disabled={
                ...
                (orderType === "Tại chỗ" &&
                  ...
                  pickupCapability.isOverloaded &&
                  !pickupOverloadConfirmed) ||
                (orderType === "Giao hàng" && !selectedDeliveryBranchId)
              }
            ...
            >
              {isSubmitting ? "Đang đẩy đơn..." : "Xác nhận đặt hàng"}{" "}
              <FiArrowRight
                style={{ marginLeft: "8px", verticalAlign: "middle" }}
              />
            </button>
                        ...
                        >
                          ...
                          >
                            ...
                            <span
                              style={{
                                fontWeight: "bold",
                                color: isEligible ? "#333" : "#d32f2f",
                              }}
                            >
                              {formatPrice(v.minordervalue || 0)}
                            </span>
                          </div>
