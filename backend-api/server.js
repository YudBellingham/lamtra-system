...
const supabaseKey =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRwd2didXRscW11YmRubm5maGRwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMyMTgwMDAsImV4cCI6MjA4ODc5NDAwMH0.N11fA3pyYUpbtGPs0yvM9lwQecM6AJIwLEnGKNswfVI";
...
Object.entries(vnp_Config).forEach(([key, value]) => {
  if (!value) {
    console.error(
      `[\x1b[31mCRITICAL ERROR\x1b[0m] Thiếu cấu hình VNPay: \x1b[33m${key}\x1b[0m trong file .env!`,
    );
  }
});
...
async function getGoongDistanceMatrix(targetLat, targetLng, branches) {
  ...
  if (!apiKey)
    return branches?.map((b) => ({ branch: b, distanceKm: Infinity })); // Safe fallback if no key
...
async function calculateIngredients(cart, supabaseClient) {
  ...
  for (const item of cart) {
    ...
    if (item.toppings && Array.isArray(item.toppings)) {
      for (const t of item.toppings) {
        if (t.toppingid && !toppingIds?.includes(t.toppingid))
          toppingIds.push(t.toppingid);
      }
    }
  }

  if (productIds.length === 0 && toppingIds.length === 0)
    return requiredIngredients;

  let allRecipes = [];
  if (productIds.length > 0) {
    const { data: pGroup } = await supabaseClient
      .from("recipes")
      .select("productid, ingredientid, amount")
      .in("productid", productIds);
    if (pGroup) allRecipes.push(...pGroup);
  }
  if (toppingIds.length > 0) {
    const { data: tGroup } = await supabaseClient
      .from("recipes")
      .select("toppingid, ingredientid, amount")
  ...
  for (const item of cart) {
    ...
    for (const r of pRecipes) {
      requiredIngredients[r.ingredientid] =
        (requiredIngredients[r.ingredientid] || 0) + r.amount * item.quantity;
    }
    ...
    if (item.toppings && Array.isArray(item.toppings)) {
      for (const t of item.toppings) {
        ...
        for (const r of tRecipes) {
          requiredIngredients[r.ingredientid] =
            (requiredIngredients[r.ingredientid] || 0) +
            r.amount * (t.quantity || 1) * item.quantity;
        }
      }
    }
  }
...
async function checkBranchCapability(
  branchId,
  cart,
  requiredIngredients,
  supabaseClient,
  ...
  const { data: bStatus } = await supabaseClient
  ...
  if (ingIds.length > 0) {
    const { data: inv } = await supabaseClient
      ...
      .select("ingredientid, currentstock")
      .eq("branchid", branchId)
      .in("ingredientid", ingIds);
  ...
  for (const item of cart) {
    ...
    if (!prodStatus || prodStatus.status !== "Còn món") {
      if (!outOfStockItems?.includes(item.name))
  ...
  if (!hasEnoughIngredients) {
    if (outOfStockItems.length === 0)
      outOfStockItems.push("Nguyên liệu theo công thức pha chế");
  }
...
app.post("/api/send-application", async (req, res) => {
  try {
    ...
    if (!fullName) {
      return res
        .status(400)
        .json({ status: "error", message: "Không có dữ liệu gửi lên." });
    }

    const storesString = Array.isArray(selectedStores)
      ? selectedStores.join(", ")
      : selectedStores || "";
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ status: "error", message: "Lỗi Resend: " + error.message });
    }

    res.json({
      status: "success",
      message: "Email ứng tuyển đã được gửi thành công!",
      data,
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ status: "error", message: "Lỗi hệ thống: " + error.message });
  }
});
...
function sortObject(obj) {
  ...
  for (let key of keys) {
    if (obj[key] !== undefined && obj[key] !== null && obj[key] !== "") {
      sorted[encodeURIComponent(key)] = encodeURIComponent(
        obj[key].toString(),
      ).replace(/%20/g, "+");
    }
  }
...
function formatVNPayDate(date) {
  ...
  const gmt7Date = new Date(
    date.getTime() + 7 * 60 * 60 * 1000 + date.getTimezoneOffset() * 60 * 1000,
  );
...
app.post("/api/create_payment_url", (req, res) => {
  try {
    ...
    console.log(
      "=== VNPAY DEBUG: CHUỖI TRƯỚC KHI BĂM (signData) ===",
      signData,
    );
    ...
    res
      .status(500)
      .json({ error: "Lỗi khi tạo URL thanh toán: " + err.message });
  }
});
  ...
  try {
    ...
    if (isDelivery) {
      ...
      if (exactLat && exactLng) {
        ...
        return res
          .status(400)
          .json({ error: "Không nhận được tọa độ giao hàng hợp lệ." });
      }
      ...
      if (!branchId)
        return res.status(400).json({ error: "Không đọc được mã chi nhánh." });
    ...
    const finalAmt =
      Math.max(0, totalAmount - calculatedDiscount) + shippingFee;
    ...
    res
      .status(500)
      .json({ error: "System processing error: " + error.message });
  }
});
...
app.post("/api/orders/reorder", async (req, res) => {
  try {
    ...
    const { data: originalOrder, error: oErr } = await supabase
      .from("orders")
      .select("*")
      .eq("orderid", orderid)
      .single();
    if (oErr || !originalOrder)
      return res.status(404).json({ error: "Không tìm thấy đơn hàng gốc." });

    if (originalOrder.status !== "Hoàn thành") {
      return res
        .status(400)
        .json({ error: "Chỉ có thể đặt lại các đơn hàng đã hoàn thành." });
    }
    ...
    const { data: orderDetails, error: odErr } = await supabase
      ...
      .select("*")
      .eq("orderid", orderid);
    if (odErr || !orderDetails || orderDetails.length === 0)
      return res
        .status(400)
        .json({ error: "Không tìm thấy chi tiết đơn hàng cũ." });
    ...
    for (const od of orderDetails) {
      ...
      const itemToppingsObj = orderToppings?.filter(
        (ot) => ot.orderdetailid === od.orderdetailid,
      );
      let validToppings = [];

      for (const ot of itemToppingsObj) {
        const topInfo = availableToppings.find(
          (t) => t.toppingid === ot.toppingid,
        );
        if (!topInfo) {
          hasMissingItems = true;
        } else {
      ...
      const priceToUse = product.saleprice
        ? product.saleprice
        : product.baseprice;
    ...
    const requiredIngredients = await calculateIngredients(
      reorderCart,
      supabase,
    );
    const capability = await checkBranchCapability(
...
app.post("/api/favorites/toggle", async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      await supabase
        .from("product_favorites")
        .insert({ customerid: cust.customerid, productid: productId });
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        templatename:
          templateName ||
          `Đơn mẫu ngày ${new Date().toLocaleDateString("vi-VN")}`,
      })
      .select();
...
// 7. Xóa đơn mẫu
app.delete("/api/saved-templates/:templateId", async (req, res) => {
  try {
    const token = req.headers.authorization?.split(" ")[1];
    if (!token) return res.status(401).json({ error: "Unauthorized" });
    const {
    ...
    const { data: template } = await supabase
      ...
      .single();
    if (!template || template.customerid !== cust.customerid) {
...
app.get("/api/vouchers/available", async (req, res) => {
  try {
    ...
    const { data: publicVouchers, error: pubErr } = await supabase
      .from("vouchers")
      .select("*")
      .eq("is_active", true)
    ...
    if (customerId) {
      const { data: customVouchers, error: custErr } = await supabase
        .from("customervouchers")
        .select("*, vouchers(*)")
        .eq("customerid", customerId)
      ...
      if (customVouchers) {
        const myVouchers = customVouchers?.map((v) => ({
          ...v.vouchers,
          cvId: v.custvoucherid,
        }));
    ...
    const uniqueVouchers = Array.from(
      new Map(finalVouchers?.map((item) => [item.voucherid, item])).values(),
    );
...
app.post("/api/orders/update-status", async (req, res) => {
  try {
    ...
    if (newStatus === "Hoàn thành" && order.voucherid && order.customerid) {
      const { data: cvToUpdate } = await supabase
        .from("customervouchers")
